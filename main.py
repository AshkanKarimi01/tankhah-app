...
tab1, tab2, tab3, tab4 = st.tabs(["📝 ثبت فاکتور", "📊 گزارش و خروجی", "✏️ ویرایش و حذف", "📜 تاریخچه"])
...
with tab1:
    with st.form("f1", clear_on_submit=True):
        c1, c2 = st.columns(2)
        d_in = c1.text_input("تاریخ (شمسی)", value=jdatetime.date.today().strftime("%Y/%m/%d"))
        cat_in = c1.selectbox("دسته بندی", CATEGORIES)
        pay_in = c1.text_input("پرداخت کننده (نام شخص)")
        ...
        if st.form_submit_button("ثبت نهایی"):
            if amt_in > 0 and pay_in.strip() != "":
                ...
                new_row = {"شماره فاکتور": next_id, "تاریخ": d_in, "دسته بندی": cat_in, "مبلغ": int(amt_in), "توضیحات": desc_in, "تصویر": img_p, "ثبت کننده": st.session_state['current_user'], "پرداخت کننده": pay_in}
                save_data(pd.concat([df, pd.DataFrame([new_row])], ignore_index=True))
                st.success(f"فاکتور {next_id} ثبت شد.")
            else: st.warning("مبلغ و نام پرداخت‌کننده الزامی است.")

# --- تب ۲: گزارش ---
with tab2:
    df_rep = load_data()
    if not df_rep.empty:
        st.dataframe(df_rep.drop(columns=["تصویر"]), use_container_width=True, hide_index=True)
        st.divider()
        sel_id = st.selectbox("مشاهده عکس شماره:", df_rep["شماره فاکتور"].tolist()[::-1])
        r = df_rep[df_rep["شماره فاکتور"] == sel_id].iloc[0]
        if r["تصویر"] != "بدون تصویر" and os.path.exists(r["تصویر"]):
            st.image(r["تصویر"], width=500)
        
        # بخش اصلاح شده دانلود اکسل بدون غلط نگارشی
        output = io.BytesIO()
...
with tab3:
    df_m = load_data()
    if not df_m.empty:
        mid = st.selectbox("انتخاب شماره فاکتور:", df_m["شماره فاکتور"].tolist())
        ...
        with c_edit:
            with st.form("f_edit"):
                ...
                if st.form_submit_button("ذخیره تغییرات"):
                    ...
                    st.rerun()
        with c_del:
            if st.button("❌ حذف کامل این فاکتور"):
                add_audit_log(mid, "حذف", "فاکتور به طور کامل پاک شد.")
                if df_m.at[idx, "تصویر"] != "بدون تصویر" and os.path.exists(df_m.at[idx, "تصویر"]):
                    os.remove(df_m.at[idx, "تصویر"])
                df_m = df_m.drop(idx)
                save_data(df_m)
                st.error("فاکتور حذف شد.")
                st.rerun()
