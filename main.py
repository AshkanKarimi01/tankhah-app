...
import io
from streamlit_jalali_date_picker import date_picker # کتابخانه تقویم شمسی
...
# --- توابع کمکی تبدیل عدد به حروف (ساده شده) ---
def num_to_farsi_text(number):
    # این یک نسخه ساده برای نمایش معادل است
    if number == 0: return "صفر"
    toman = number // 10
    return f"معادل {toman:,} تومان"
...
def check_password():
    ...
    if st.button("ورود"):
        users = {"barjani": "1234", "talebi": "1234"} # تغییر یوزر طبق درخواست شما
...
st.info(f"💰 **موجودی فعلی (ریال):** {balance:,} | {num_to_farsi_text(balance)}")
...
UNITS = ["انبار", "فروش", "مالی", "اداری", "هیات مدیره", "مشاور", "بازرگانی", "ممیز", "فناوری اطلاعات"] # واحد جدید اضافه شد
...
with tab1:
    with st.form("main_form", clear_on_submit=True):
        ...
        with c1:
            ...
            d_fact = date_picker(key='fact_date') # تقویم شمسی
            st.write("تاریخ پرداخت:")
            d_pay = date_picker(key='pay_date') # تقویم شمسی دوم
        ...
        with c2:
            ...
            st.caption(f"✍️ {num_to_farsi_text(amt_in)}") # نمایش معادل فارسی
        ...
        if st.form_submit_button("ثبت نهایی"):
            ...
            new_row = {
                ...
                "دسته بندی": cat_in, "واحد": unit_in, "مبلغ": amt_in, 
...
with tab2:
    if not df_exp.empty:
        st.subheader("فیلتر تاریخ پرداخت:")
        c1, c2 = st.columns(2)
...
with tab3:
    with st.form("inc_form", clear_on_submit=True):
        st.subheader("افزایش موجودی (ریال)")
        i_amt = st.number_input("مبلغ واریزی به ریال", min_value=0)
        st.caption(num_to_farsi_text(i_amt))
